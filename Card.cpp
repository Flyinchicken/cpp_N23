...
void Card::play(Hand* hand) {
    ...
    if (isRemoved) { //If the Card was removed 
        ...
        hand->getPlayer()->issueOrder(generateOrderNumber(_type)); //Calls member of the Player class which issues an order based on the type of the card and adds it to its list
    }
...
Hand::Hand(const Hand& e) {
    ...
    this->_player = new Player(new string(e._player->getName()));
};
